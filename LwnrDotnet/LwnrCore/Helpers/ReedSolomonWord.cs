...
public static class ReedSolomonWord
{
    ...
    private static int[] rsErrorLocatorPoly(int[] synd, int sym, int erases)
    {
        ...
        for (var i = 0; i < (sym - erases); i++)
        {
            ...
            {
                delta ^= gfMul(errLoc[^(j + 1)], synd[kappa - j]);
            }
            ...
            if (oldLoc.Length > errLoc.Length)
            {
                ...
                oldLoc = gfPolyMulScalar(errLoc.ToArray(), gfInverse(delta));
                errLoc = newLoc;
            }

            var scale = gfPolyMulScalar(oldLoc.ToArray(), delta);
            errLoc = gfAddPoly(errLoc.ToArray(), scale.ToArray());
        }

        errLoc = removeLeadingZeros(errLoc.ToArray());
        var errCount = errLoc.Length - 1;
